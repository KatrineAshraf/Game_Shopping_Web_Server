...
var con = mysql.createConnection({
 ...
 var sql = "CREATE TABLE customers (id INT AUTO_INCREMENT PRIMARY KEY, fname VARCHAR(25) NOT NULL, lname varchar(25) not null, email varchar(50) not null unique,  balance DECIMAL(10,2) not null default 0.00, password varchar(8));"
db.query(sql, function (err, data, fields) {
    if (err) throw err;
    console.log("Table Customers Created!")
  }) 

  var sql = "CREATE TABLE products (id INT AUTO_INCREMENT PRIMARY KEY, p_name Varchar(25), image Varchar(300), price DECIMAL(10,2), qty INT);"
  db.query(sql, function (err, data, fields) {
      if (err) throw err;
...
SELECT id, concat_ws(' ', fname, lname) as "name", email, balance
  from customers; 
  INSERT INTO products (p_name, image, price, qty) VALUES
    (
        "Die Macher",
    ...
    create table orders (id INT AUTO_INCREMENT PRIMARY KEY, c_id INT NOT NULL, p_id INT NOT NULL, ordered_at timestamp default current_timestamp, state int not null default 0);
insert into orders (c_id, p_id) values (1,8);
create view transactions as
select  orders.id, concat_ws(' ', fname, lname) as "name", products.image, products.p_name as product, products.price, DATE_FORMAT(orders.ordered_at, "%d/%c/%Y @ %r") as ordered_at, orders.state
from ((orders
